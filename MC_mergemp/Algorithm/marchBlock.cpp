	...
	for (unsigned zidx = ext[4]; zidx <= ext[5]; ++zidx, zpos += spacing[2]) {
		...
		for (unsigned yidx = ext[2]; yidx <= ext[3];
			...
			for (unsigned xidx = ext[0]; xidx <= ext[1]; ++xidx, xpos +=
				...
				val[0] = X1buffer[xidx-ext[0]];
				val[1] = X1buffer[xidx-ext[0]+1];

				val[2] = X2buffer[xidx-ext[0]+1];
				val[3] = X2buffer[xidx-ext[0]];

				val[4] = X3buffer[xidx-ext[0]];
				val[5] = X3buffer[xidx-ext[0]+1];

				val[6] = X4buffer[xidx-ext[0]+1];
				val[7] = X4buffer[xidx-ext[0]];
				...
				for (; *edges != -1; edges += 3) {
					...
					for (int i = 0; i < 3; ++i) {
						...
						unsigned edgeIndex = edgeIndices.getEdgeIndex(xidx, yidx,
								zidx, edges[i]);
						if (edgeIndex==45421932) {
							CLOG(logDEBUG) << "Found edge: 45421932==================";
