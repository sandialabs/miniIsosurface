...
template<typename T>
void computeGradient(unsigned xidx, unsigned yidx, unsigned zidx, const Image3D<T>* imageVolume, T grad[3]) {
	T x[3][2];
	imageVolume->getValsForGradient(x,xidx,yidx,zidx);

	const T* spacing=imageVolume->getSpacing();
...
template<typename T>
void computeAllGradients (unsigned &xidx, unsigned &yidx, unsigned &zidx, const Image3D<T>* imgVol, T (& grad)[8][3]) {
...
template static void computeGradient<float_t> (unsigned xidx, unsigned yidx, unsigned zidx, const Image3D<float_t>*,	float_t grad[3]);

template void computeAllGradients<float_t> (unsigned &xidx, unsigned &yidx, unsigned &zidx, const Image3D<float_t>*, float_t (& grad)[8][3]);
